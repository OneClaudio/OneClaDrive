...
ssize_t freadfull( buf, size, nblocks, fd){
	ssize_t nread=0;
	
	nread=fread( buf, size, nblocks, fd);
	if(nread<size){
		if( feof(fd) ) fprintf(stderr, "Error: Unexpected EOF in fread\n");
		if(ferror(fp)) fprintf(stderr, "Error: Reading error in fread\n");
		errno=EIO;
		return -1;
		}
	return 0;
...
ssize_t fwritefull( buf, size, nblocks, fd){
	ssize_t nwritten=0;
